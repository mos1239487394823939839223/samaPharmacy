...
`#` · كود الصنف · إسم الصنف · الكمية · الوحدة · تاريخ الصلاحية · الرصيد · السعر · قيمة ق.خ · قيمة ن.خ · خصم · delete
...
- **Barcode scanner** — a keyboard wedge. Detect scans by inter-keystroke timing (under ~30ms between characters, terminated by Enter) rather than a dedicated input. This lets scanning work from anywhere on the POS screen.
- **Receipt printer** — `node-thermal-printer` for ESC/POS in the main process. Faster and more reliable than rendering HTML and calling `webContents.print()`. Keep an HTML A5 path for full invoices.
- **Cash drawer** — opened by the printer's kick-out command, not by a separate driver.
